...
const contentSources = {
  ...
  xhsHongqiaoTaxi: { label: "小红书 · 虹桥 T2 网约车上车经验（2026-03 至 06，仅作经验旁证）", url: "https://www.xiaohongshu.com/search_result?keyword=%E8%99%B9%E6%A1%A5T2%20%E6%89%93%E8%BD%A6%20%E6%94%BB%E7%95%A5&source=web_explore_feed", kind: "经验旁证" as const },
  xhsPudongTaxi: { label: "小红书 · 浦东机场网约车上车经验（2026-05，仅作经验旁证）", url: "https://www.xiaohongshu.com/search_result?keyword=%E6%B5%A6%E4%B8%9C%E6%9C%BA%E5%9C%BA%20%E6%89%93%E8%BD%A6%20%E6%94%BB%E7%95%A5&source=web_explore_feed", kind: "经验旁证" as const },
};
...
export const guideCards: GuideCard[] = [
  {
    ...
    steps: [
      "拿到行李后，先打开“最晚上车时间查询”：起点选实际的虹桥 1 号或 2 号航站楼，终点选“同济大学”；它才会把整段线路算进去。",
      "给最后一班留 15 分钟缓冲：T2 预计无法在 22:15 前进闸、T1 无法在 22:18 前进闸，就直接切到打车，不再拖着行李赶。",
    ...
    backup: "过了安全进闸线、行李很多或身体疲惫时，直接看下方虹桥打车卡；也可在到达层服务台确认官方夜间交通。",
    verifiedAt: guideVerifiedAt,
    freshness: "请实时查询",
    actionLabel: "查最晚上车时间",
    actionUrl: "https://service.shmetro.com/zuiwantime/index.htm",
  ...
  },
  {
    ...
    steps: [
      "拿到行李后立即用上海地铁“最晚上车时间查询”：起点选“浦东 1 号 2 号航站楼”，终点选“同济大学”，不要只查 2 号线单线。",
    ...
    backup: "过了安全进闸线、航班晚点或人很累时，直接看下方浦东打车卡；机场巴士可作到市区的备选，但不直达同济。",
    verifiedAt: guideVerifiedAt,
    freshness: "请实时查询",
    actionLabel: "查最晚上车时间",
    actionUrl: "https://service.shmetro.com/zuiwantime/index.htm",
  ...
  },
  {
    id: "hongqiao-taxi",
    section: "arrival",
    title: "虹桥想打车：让平台和现场一起指路",
    summary: "不要照搬“水果层”“酒店门口”等旧攻略。虹桥 T2 的停车库动线会施工调整；平台分配的上车层和现场“网约车”指示，才是当下该信的。",
    steps: [
      "取完行李后，在平台里确认自己选的是实际到达的虹桥 T1 或 T2，不要误选“虹桥 2 号航站楼地铁站”。",
      "跟着航站楼内“网约车”指示走；到接驳区/停车库后，再按平台此刻给出的楼层、区域和车牌找车。",
      "上电梯前先看一次平台的楼层，抵达后再核一次车牌、车型和司机姓名；不一致就不上车。",
      "平台长时间没人接或预估价异常时，先取消平台订单，改走机场正规出租车排队区；不接受主动揽客的私下报价。",
    ],
    time: "上车点和等候时间会随航站楼施工、客流和平台调度变化；不要为找一个旧攻略里的固定点位绕远路",
    tip: "小红书上“P7 水果层/周边酒店”都有成功和失败案例，连楼层标识也可能变化；它们只能帮你理解为什么要看实时分配，不能替代实时分配。",
  ...
  },
  {
    id: "pudong-taxi",
    section: "arrival",
    title: "浦东想打车：先到指定区，再按订单找车",
    summary: "浦东 T2 的经验帖普遍指向“网约车 / P2 停车楼”，但 B1、B2、G1 等具体楼层会由平台实时分配；不追旧帖里的固定编号。",
    steps: [
      "取完行李后跟“网约车 / P2 停车楼”指示走；T1、T2 的动线不同，先认航站楼再动。",
      "到电梯前打开平台，先确认它此刻分配的楼层和区域，再下到对应层；不要先按网上的 G1、B1 或 B2 猜。",
      "接近指定上车区再叫车或确认订单，避免司机过早进入、产生等待后又错过彼此；到点后只按订单核车牌上车。",
      "有人在路上主动问“要不要车”时不私下成交；平台连续无车或价格不合适，就走机场正规出租车排队区。",
    ],
    time: "T2 从到达层走到 P2 常被经验帖描述为约 5 分钟，但人流、停车库楼层和当日调度会改变实际时间",
    tip: "经验帖中同一航站楼就出现过 G1、B1、B2 的不同说法，评论也提示现场会变；所以本卡只固定“看网约车/P2指示 + 看订单实时楼层”这条不易过期的流程。",
